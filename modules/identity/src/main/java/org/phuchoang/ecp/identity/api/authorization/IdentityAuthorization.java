...
/**
 * The authorization Open Host Service (`US-AUD-03`, `ADR-0016` §4, `Domain Model.md` §5.2),
 * exposed for every other module's <strong>application</strong> layer to call before executing a
 * command — the identical decision reached regardless of entry point (`BR-AUD-02`), because a
 * controller, a scheduled job, and a Kafka consumer all reach business logic through an
 * application service, and this is the one place the rule is enforced.
 *
