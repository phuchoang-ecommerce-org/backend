...
public interface CatalogAdministrationFacade {
    ...
    ProductDetailView createProduct(IdentityActor caller, UUID correlationId, ProductWrite request);
    /** Replaces the mutable merchandising fields of an existing product. */
    ProductDetailView updateProduct(IdentityActor caller, UUID correlationId, UUID productId, ProductWrite request);
    /** Discontinues a product so it is no longer available to new catalog reads or purchases. */
    void deleteProduct(IdentityActor caller, UUID correlationId, UUID productId);
    /** Transitions a product's publication state and records the supplied reason. */
    ProductDetailView setPublication(IdentityActor caller, UUID correlationId, UUID productId, PublicationWrite request);
    /** Adds one SKU-bearing variant while preserving catalog-wide SKU uniqueness. */
    VariantView addVariant(IdentityActor caller, UUID correlationId, UUID productId, VariantWrite request);
    /** Removes a variant from a product's purchasable configuration set. */
    void removeVariant(IdentityActor caller, UUID correlationId, UUID productId, UUID variantId);
    /** Replaces a variant's current list price without rewriting historical order prices. */
    VariantView changePrice(IdentityActor caller, UUID correlationId, UUID productId, UUID variantId, PriceWrite request);
    /** Adds an ordered display image owned by the product aggregate. */
    ProductImageView addImage(IdentityActor caller, UUID correlationId, UUID productId, ImageWrite request);
    /** Removes an aggregate-owned display image. */
    void removeImage(IdentityActor caller, UUID correlationId, UUID productId, UUID imageId);
    /** Creates a category while enforcing the hierarchy invariants of {@code BR-CAT-03}. */
    CategoryView createCategory(IdentityActor caller, UUID correlationId, CategoryWrite request);
    /** Updates a category and rejects an attempted cycle in its parent hierarchy. */
    CategoryView updateCategory(IdentityActor caller, UUID correlationId, UUID categoryId, CategoryWrite request);
    /** Deletes an empty category only; callers must first resolve its products and child categories. */
    void deleteCategory(IdentityActor caller, UUID correlationId, UUID categoryId);
    /** Applies independent product amendments and returns the outcome for every submitted item. */
    BulkResult amendBulk(IdentityActor caller, UUID correlationId, List<BulkItem> items);
